...
量化交易系统 API，基于 NestJS 11 + TypeORM + PostgreSQL + Redis 构建，提供用户认证（JWT 双密钥方案）。
...
- 模块的 dto 相关的放到 `模块/dto` 目录下，每个接口的 dto 对应一个文件；
  模块级共享的枚举/常量放 `模块/模块.constants.ts`
- DTO 校验优先复用组合装饰器，避免在各 DTO 中重复堆砌校验装饰器
  （当前项目暂无公共装饰器，需要跨模块复用时在 `common/` 下新建）
