import { EMPTY_MESSAGES, ERROR_PREFIX } from './cli.constants'
import type { TableParseResult, TableRow } from './cli.types'

/**
 * CLI 输出解析器（纯函数，无 Nest 依赖）
 *
 * 这些 CLI 输出一张扁平的 Markdown 表格（**没有分组标题**），列名随命令变化，
 ...
 *
 * 判定顺序（短路）：
 * 1. 空输出 → `empty`（实测 clawhub 的 `--sector` 即如此：无任何输出且退出码 0）
 * 2. 以「执行失败」开头 → `invalid`
