 ...
 *
 * **限流**：在全局默认（每 60 秒 60 次）之上按接口差异化——
 * 注册/改密每小时 5 次、登录每 10 分钟 10 次、刷新每 10 分钟 20 次；
 * 两个登出接口用 `@SkipThrottle` 豁免（登出是幂等的清理动作）。
 * 登录另有 `AuthService` 的失败锁定（连续 5 次失败锁 15 分钟），与限流是两套独立机制。
 *
 * **方法顺序**：register → login → refresh → logout → logout-all → change-password → profile
 * （`REST_CLIENT.http`、`docs/api-auth.md`、`docs/api-config.md` 遵循同一顺序）。
 */
@Controller('auth')
