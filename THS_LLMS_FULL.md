...
按 **基础数据 + A股** 双主干组织：基础数据提供标的检索与标的列表获取，A股主干覆盖交易日历、行情、除复权、财务报表、指数数据、涨跌停数据与全市场数据导出。

- [REST API](/docs/api-reference/overview): 标准 HTTP 接口，按 `/api/<标的宇宙>/<数据类型>/<动作>` 形态组织。覆盖行情快照、
  历史 K 线、标的检索 / 列表、除复权、财务报表、指数数据、涨跌停数据与全市场 Parquet 导出。
...
- [A 股 · 除复权](/docs/api-reference/corporate-actions): 单只 A 股的原始复权因子事件流（现金分红 / 送股 / 配股），供客户端自行推导复权因子。
...
- [A 股 · 同花顺功能数据](/docs/api-reference/special-data): 提供涨跌停、热榜、个股异动原因与龙虎榜数据，面向盘面复盘和短线情绪分析。
- [元信息 · 标的检索](/docs/api-reference/ticker-search): 按 thscode / ticker / 名称做跨市场消歧检索。
- [元信息 · 标的列表获取](/docs/api-reference/ticker-list): 按市场与资产类型分页获取标的列表。
- [统一响应信封](/docs/api-reference/overview): 所有接口返回 `ApiResponse`（`code` / `message` / `request_id` / `data`），业务错误经 `code` 表达。

## 提供给 AI 助手的聚合文档

如果你希望让 ChatGPT、Claude、Cursor 等 AI 助手快速理解本站接口，可以直接提供以下公开地址：

- [llms.txt · 文档索引](https://fuyao.aicubes.cn/llms.txt): 全站文档的结构化索引，适合让 AI 先了解有哪些 REST API、MCP 工具和使用指南。
- [llms-full.txt · 文档全文](https://fuyao.aicubes.cn/llms-full.txt): 全站文档的完整聚合，适合直接加入 AI 上下文，以便查询参数、字段和调用示例。

这两份文件会随站点文档自动更新。需要完整接口细节时优先使用
[llms-full.txt](https://fuyao.aicubes.cn/llms-full.txt)；只需浏览文档目录时使用
...
我们将 A 股市场的行情、除复权、财务报表、交易日历、指数行情、同花顺指数、涨跌停数据与跨市场标的元信息整理为低延迟、易解析的结构化字段，
通过统一的响应信封对外暴露，方便接入方直接喂模型、做回测或驱动前端图表。当前目录采用 **基础数据 + A股** 双主干：基础数据负责标的检索与标的列表获取，A股主干聚合已上线的行情、财务、指数与特色功能数据。

## 提供的数据

| 数据域                                                           | 内容                                                                | 说明                                               |
| ---------------------------------------------------------------- | ------------------------------------------------------------------- | -------------------------------------------------- |
| [元信息 · 标的检索](/docs/api-reference/ticker-search)           | 跨市场标的消歧检索                                                  | 按 thscode / ticker / 名称解析标准标的。           |
| [元信息 · 标的列表获取](/docs/api-reference/ticker-list)         | 代码表批量获取                                                      | 按市场与资产类型分页获取标的列表。                 |
...
| [A 股 · 市场数据导出](/docs/api-reference/market-dumps)          | 全 A 股 10 年日 K、最近 10 交易日日 K 与全历史复权因子 Parquet 文件 | 通过预签名链接一次性拉全市场数据。                 |
| [A 股 · 除复权](/docs/api-reference/corporate-actions)           | 现金分红、送股、配股原始事件流                                      | 客户端可自行推导复权因子。                         |
| [A 股 · 财务报表](/docs/api-reference/financials)                | 利润表 / 资产负债表 / 现金流量表多期序列                            | 支持最近 N 期或时间区间取数。                      |
| [A 股 · 指数行情与同花顺指数](/docs/api-reference/a-share-index) | 指数行情快照 / 历史 K 线、同花顺指数列表与成分股                    | 同时支持沪深 300 等标准指数的成分股查询。          |
| [A 股 · 涨跌停数据](/docs/api-reference/limit-up-data)           | 涨停股票池与连板天梯                                                | 面向盘面复盘和短线情绪分析。                       |
...
3. 在请求头中携带 `X-api-key: <your-api-key>` 即可访问 REST API；MCP 接入同样使用该 API Key。
...
- **量化研究 / 策略平台** —— 行情快照 + 历史 K 线 + 复权事件 + 指数成分，覆盖回测、选股与实时打分。
- **Fintech / 投研产品** —— snake_case 字段直接驱动前端图表、表格、指数/板块分析与复盘页面。
...
在请求头中携带 `X-api-key: <your-api-key>`。以行情快照为例：
...
公开 REST 与对应 MCP Tool 共享同一套后端 capability，数据语义完全一致；部分数据与分析能力已经接入同花顺AI客户端，请前往使用。
...
| [主力资金](./capital-flow.mdx)                                | `/api/a-share/capital-flow`                                         | A 股主力资金实时快照与历史数据，已经接入同花顺AI客户端，请前往使用。                       |
| [高频动向](./high-frequency.mdx)                              | `/api/a-share/high-frequency`                                       | A 股个股与指数的高频历史和单日分时数据，已经接入同花顺AI客户端，请前往使用。               |
| [全市场数据导出 (market-dumps)](./market-dumps.mdx)           | `/dump/market-dumps`（浏览器）/ `/api/dump/market-dumps`（API Key） | 价格数据子模块：A 股全市场 10 年日 K、最近 10 交易日日 K 与复权因子 Parquet 文件下载链接。 |
| [资讯事件库](./news-events.mdx)                               | `/api/news/events/search`                                           | 按标题、正文、时间、评级与关联板块检索资讯事件，仅在同花顺AI客户端内提供。                 |
| [标的检索](./ticker-search.mdx)                               | `/api/meta/tickers/search`                                          | 按 thscode / ticker / 名称检索证券、基金、期货与期权标的。                                 |
| [标的列表获取](./ticker-list.mdx)                             | `/api/meta/tickers/list`                                            | 按规范化资产类型分页获取代码表。                                                           |
| [基金](./funds.mdx)                                           | `/api/fund`                                                         | 基金资料、持仓、业绩、经理、财务、诊断、募集、资讯与行情数据。                             |
| [基金在线回测](./fund-backtest.mdx)                           | `/api/fund/backtest`                                                | 执行基金在线回测并查询可用指标。                                                           |
| [基金通用指标](./fund-indicators.mdx)                         | `/api/fund/indicators`                                              | 查询基金画线式与表格式指标。                                                               |
| [QDII 额度](./fund-quota.mdx)                                 | `/api/fund/quota`                                                   | 查询 QDII 分类额度汇总与基金列表。                                                         |
| [期货总览](./futures-overview.mdx)                            | `/api/futures`                                                      | 期货品种、合约、持仓、仓单、基差、基本面、交易日与行情。                                   |
| [期货F10宏观指标历史数据](./futures-fundamentals.mdx)         | `/api/futures/fundamentals`                                         | 期货 F10 宏观指标历史数据，计划在后续版本接入同花顺AI客户端，敬请期待。                    |
| [期货交易时间轴](./futures-session-timeline.mdx)              | `/api/futures/calendar/session-timeline`                            | 期货合约最近交易日的交易时间轴。                                                           |
| [期权总览](./options-overview.mdx)                            | `/api/options`                                                      | 期权品种、合约、分时行情、K 线与交易时间轴。                                               |
| [期权交易时间轴](./options-session-timeline.mdx)              | `/api/options/calendar/session-timeline`                            | 期权合约最近交易日的交易时间轴。                                                           |
| [除复权 (corporate-actions)](./corporate-actions.mdx)         | `/api/a-share/corporate-actions`                                    | A 股复权因子事件流（分红 / 送股 / 配股）。                                                 |
| [财务报表 (financials)](./financials.mdx)                     | `/api/a-share/financials`                                           | A 股整体合并利润表 / 资产负债表 / 现金流量表多期序列。                                     |
| [财务指标数据](./financial-indicators.mdx)                    | `/api/a-share/financials/indicators`                                | A 股成长、盈利、偿债、营运、现金流五类财务指标。                                           |
| [交易日历 (calendar)](./calendar.mdx)                         | `/api/a-share/calendar`                                             | A 股近一年交易日序列（含毫秒戳与 `yyyyMMdd`）。                                            |
| [集合竞价数据](./auction.mdx)                                 | `/api/a-share/auction`                                              | A 股集合竞价快照与短线风向标竞价基准。                                                     |
| [特色数据 (special-data)](./special-data.mdx)                 | `/api/a-share/special-data`                                         | A 股涨跌停数据、同花顺热榜、个股异动原因与龙虎榜。                                         |
...
已注册的公开 REST capability 同时暴露为 MCP Tools，供 AI Agent 调用。
...
全市场数据导出是价格数据模块的便捷入口，用于一次性获取全市场历史或近期行情，
...
三种 dump 共享同一套元信息约定，按 `dump_id` 区分：
...
日 K dump 共用相同的 Parquet schema；`daily-k` 提供最近约 10 年全量窗口，`daily-k-10d`
提供最近 10 个交易日的轻量增量窗口。复权因子 dump 使用独立的事件流 schema。
...
导出 CSV、检查重复键等。脚本会根据 schema 自动识别日 K 与复权因子 dump，因此三种下载文件均可直接使用。
...
#!/usr/bin/env python3
"""
Read and inspect Fuyao market-dump Parquet files for all publishable kinds:

  - daily-k             rows keyed by (thscode, date_ms),     dump_id a_share_daily_k_1d_none_10y
  - daily-k-10d         rows keyed by (thscode, date_ms),     dump_id a_share_daily_k_1d_none_10d
...
Examples (daily-k):
  python3 scripts/read_market_dump.py build/dump/a_share_daily_k_1d_none_10y/20260602/manifest.json
  python3 scripts/read_market_dump.py build/dump/a_share_daily_k_1d_none_10y/20260602 --ticker 600519.SH
  python3 scripts/read_market_dump.py build/dump/a_share_daily_k_1d_none_10d/20260602 --check-duplicates
  python3 scripts/read_market_dump.py build/dump/a_share_daily_k_1d_none_10d/20260602 --export-csv /tmp/sample.csv
...
Inspecting a file downloaded via GET /dump/market-dumps/<kind>/download-url:
...
来源：https://fuyao.aicubes.cn/docs/api-reference/ticker-search/

> 按 thscode / ticker / 名称检索证券、基金、期货与期权标的
...
来源：https://fuyao.aicubes.cn/docs/api-reference/ticker-list/
...
[A 股行情快照](/docs/api-reference/prices#响应字段) 一致。
...
[A 股历史 K 线](/docs/api-reference/prices#响应字段-1) 一致。指数接口响应中的 `data.adjust`
固定为 `null`。

## 基金总览

来源：https://fuyao.aicubes.cn/docs/api-reference/funds/

> 基金资料、持仓、业绩、经理、财务、资讯与行情数据 API 总览

基金模块提供基金资料、持仓、业绩、持有人、经理、公司、财务、诊断、募集、资讯与行情数据查询。
基金业务接口使用 `thscode` 唯一定位基金；经理、公司接口分别使用 `manager_id`、`company_id`，行情快照支持 ETF/LOF，历史日线仅支持 ETF。`thscode` 必须保留市场后缀。
...
| [基金业绩与回撤](./fund-performance.mdx)     | `/api/fund/performance`        | 查询基金净值、收益、净值波动、趋势强弱、估值百分位与最大回撤。 |
...
| [基金行情数据](./fund-market.mdx)            | `/api/fund/market`             | 查询场内基金行情快照与场内基金前复权历史日线行情。             |

## 通用约定

- **Base URL**：`https://fuyao.aicubes.cn`。
- **必需请求头**：`X-api-key`，缺失、无效或无权访问时返回认证/权限错误码。
- `thscode`：基金唯一标识，必须使用包含市场后缀的完整同花顺代码，例如 `025480.OF`、`510300.SH`、`161725.SZ`。
...
| code   | 含义                 | 基金场景                                      |
| ------ | -------------------- | --------------------------------------------- |
| `1001` | 缺少必填参数         | 缺少 `thscode`、`start`、`end` 等必填参数。   |
...
来源：https://fuyao.aicubes.cn/docs/api-reference/special-data/

> A 股特色数据：涨跌停与炸板数据、同花顺热榜、个股异动原因与龙虎榜

特色数据模块提供 A 股盘面加工类数据，覆盖涨跌停数据、同花顺热榜、个股异动原因和龙虎榜。
返回统一响应信封 `ApiResponse`，业务错误经 `code` 字段表达，HTTP 状态码恒为 200。
...
来源：https://fuyao.aicubes.cn/docs/api-reference/anomaly-analysis/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/auction/
...
通用鉴权、响应信封与错误码参见 [API 参考](./overview.mdx)。

## 主力资金

来源：https://fuyao.aicubes.cn/docs/api-reference/capital-flow/
...
<div className="ai-client-notice">
  <div className="ai-client-notice__icon" aria-hidden="true">
    <svg viewBox="0 0 24 24" role="img">
      <path d="M4 18h16M6 15l4-4 3 2 5-7M18 6h-4M18 6v4" />
    </svg>
  </div>
  <div className="ai-client-notice__content">
    **该能力暂未开放外部接入**
    <span>相关数据能力已经接入同花顺AI客户端，请前往使用。</span>
  </div>
  <a className="ai-client-notice__action" href="https://lumi.10jqka.com.cn/ai_data/oss/preview/lumi/lumi-shejitest/lumi-landing-page/index.html?channel=Hithink-API" target="_blank" rel="noopener noreferrer">前往同花顺AI客户端 ↗</a>
</div>
...
- 所有接口返回统一 `ApiResponse` 信封；时间戳为毫秒级 Unix 时间戳，时间窗按 `Asia/Shanghai` 判断。
...
```json
{
  ...
  "data": {
    ...
    "super_large": {
      ...
      "outflow_amount": 100000000.0,
      "net_amount": 23456789.12
    },
    "large": {
      "inflow_amount": 86543210.5,
...
来源：https://fuyao.aicubes.cn/docs/api-reference/dragon-tiger-data/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/financial-indicators/
...
MCP Tool：[`get_a_share_financials_indicators`](/docs/mcp/tools/get_a_share_financials_indicators)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-backtest/
...
- **Base URL**：`https://fuyao.aicubes.cn`。
- **必需请求头**：`X-api-key`；认证、权限和业务错误通过响应 `code` 表达。
- 本页接口返回统一 `ApiResponse` 信封，包含 `code`、`message`、`data` 与 `request_id`。
...
**MCP Tool**：[`get_fund_backtest_result`](/docs/mcp/tools/get_fund_backtest_result)
...
**MCP Tool**：[`get_fund_backtest_indicators`](/docs/mcp/tools/get_fund_backtest_indicators)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-company/
...
通用鉴权与错误码参见[基金 API 总览](./funds.mdx)。

## 基金分红记录

来源：https://fuyao.aicubes.cn/docs/api-reference/fund-corporate-actions/
...
通用参数与错误码参见[基金 API 总览](./funds.mdx)。

## 基金诊断详情

来源：https://fuyao.aicubes.cn/docs/api-reference/fund-diagnostics/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-financials/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-holders/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-holdings/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-indicators/
...
**MCP Tool**：[`get_fund_indicators_line`](/docs/mcp/tools/get_fund_indicators_line)
...
```json
{
  ...
  "data": {
    "time_range": [
      1788192000000, 1788278400000, 1788364800000, 1788451200000, 1788710400000, 1788796800000
    ],
    "indexes": [
      {
        "index_id": "rsi_pct",
    ...
    "data": [
      {
        ...
        "values": [
          {
            "idx": 0,
            "values": [43.29, 39.94, 27.61, 23.6, 43.48, 40.7]
          }
        ]
      }
    ]
...
**MCP Tool**：[`get_fund_indicators_table`](/docs/mcp/tools/get_fund_indicators_table)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-managers/
    ...
    "item": [
      {
        ...
        "industry_preferences": [
          {
            "report_tag": "2022",
            "percent": [0.07, 0.26, 0.11, 0.27, 0.05, 0, 0.23],
...
以上接口的 `timestamp` 均为接口响应时间戳。通用错误码参见[基金 API 总览](./funds.mdx)。

## 基金行情数据

来源：https://fuyao.aicubes.cn/docs/api-reference/fund-market/

> 查询场内基金行情快照与场内基金前复权历史日线行情
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-news/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-offerings/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-performance/

> 查询基金净值、区间收益、净值波动、趋势强弱、估值百分位与最大回撤
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-portfolio/
...
以上接口的 `timestamp` 均为接口响应时间戳。通用参数与错误码参见[基金 API 总览](./funds.mdx)。

## 基金基本资料

来源：https://fuyao.aicubes.cn/docs/api-reference/fund-profile/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/fund-quota/
...
**MCP Tool**：[`get_fund_quota_summary`](/docs/mcp/tools/get_fund_quota_summary)
...
**MCP Tool**：[`get_fund_quota_list`](/docs/mcp/tools/get_fund_quota_list)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-basis/
...
## 期货主连最新基差 {#basis-main-continuous-latest}

```text
GET /api/futures/basis/main-continuous-latest
```

MCP Tool：[`get_futures_basis_main_continuous_latest`](/docs/mcp/tools/get_futures_basis_main_continuous_latest)
...
## 期货历史基差 {#basis-historical}

```text
GET /api/futures/basis/historical
```

MCP Tool：[`get_futures_basis_historical`](/docs/mcp/tools/get_futures_basis_historical)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-calendar/

> 查询期货合约在日期范围内的交易日及对应交易时间安排
...
MCP Tool：[`get_futures_calendar_trading_schedule`](/docs/mcp/tools/get_futures_calendar_trading_schedule)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-contracts-extended/

> 查询期货主连、主力、次主力与商品指数合约资料
...
- **Base URL**：`https://fuyao.aicubes.cn`。
- **必需请求头**：`X-api-key`；认证与权限错误通过业务 `code` 表达。
- 接口返回统一 `ApiResponse` 信封；ISO 日期按 `Asia/Shanghai` 解释，Unix 时间戳为毫秒。
- 期货合约使用完整 `thscode`，品种列表每次最多 5 项；金融数值与日期可为 `null`，合法无数据返回空数组。
  :::

## 期货主连资料 {#futures-main-continuous}

```text
GET /api/futures/contracts/main-continuous-list
```

MCP Tool：[`get_futures_contracts_main_continuous_list`](/docs/mcp/tools/get_futures_contracts_main_continuous_list)
...
## 期货主力合约 {#futures-main-contracts}

```text
GET /api/futures/contracts/main-list
```

MCP Tool：[`get_futures_contracts_main_list`](/docs/mcp/tools/get_futures_contracts_main_list)
...
## 期货次主力合约 {#futures-secondary-main-contracts}

```text
GET /api/futures/contracts/secondary-main-list
```

MCP Tool：[`get_futures_contracts_secondary_main_list`](/docs/mcp/tools/get_futures_contracts_secondary_main_list)
...
字段结构与[期货主力合约](#futures-main-contracts)一致；合法无数据返回空数组。

## 商品指数合约列表 {#futures-commodity-indexes}

```text
GET /api/futures/contracts/commodity-index-list
```

MCP Tool：[`get_futures_contracts_commodity_index_list`](/docs/mcp/tools/get_futures_contracts_commodity_index_list)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-fundamentals/

> 仅供同花顺AI客户端内使用的期货 F10 宏观指标历史数据
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-overview/

> 期货基础资料、持仓、仓单、基差、交易日与行情 API 总览

期货模块提供品种、合约、持仓、仓单、基差、交易日与行情数据查询。

## 功能分类

| 分类                                                   | 路径前缀                                 | 说明                                                                     |
| ------------------------------------------------------ | ---------------------------------------- | ------------------------------------------------------------------------ |
| [期货品种资料](./futures-varieties.mdx)                | `/api/futures/varieties`                 | 查询期货品种目录及交易属性。                                             |
| [期货品种板块](./futures-variety-plates.mdx)           | `/api/futures/variety-plates`            | 查询期货品种所属板块。                                                   |
| [期货合约基础资料](./futures-reference.mdx)            | `/api/futures/contracts`                 | 查询期货合约详情和分页合约目录。                                         |
| [期货主力与指数合约](./futures-contracts-extended.mdx) | `/api/futures/contracts`                 | 查询主连、主力、次主力与商品指数合约列表。                               |
| [期货持仓](./futures-positions.mdx)                    | `/api/futures/positions`                 | 查询品种、公司和合约维度的日持仓与历史持仓。                             |
| [期货仓单](./futures-warehouse-receipts.mdx)           | `/api/futures/warehouse-receipts`        | 查询期货历史仓单。                                                       |
| [期货基差](./futures-basis.mdx)                        | `/api/futures/basis`                     | 查询主连最新基差与历史基差。                                             |
| [期货F10宏观指标历史数据](./futures-fundamentals.mdx)  | `/api/futures/fundamentals`              | 仅供同花顺AI客户端内访问，端外暂不开放。                                 |
| [期货交易日](./futures-calendar.mdx)                   | `/api/futures/calendar`                  | 查询交易日列表及对应交易时间安排。                                       |
| [期货行情](./futures-prices.mdx)                       | `/api/futures/prices`                    | 端外查询当前交易日分时与日 K；AI 客户端内还可查询历史分时与多周期 K 线。 |
| [期货交易时间轴](./futures-session-timeline.mdx)       | `/api/futures/calendar/session-timeline` | 查询期货合约最近交易日的交易时间轴。                                     |

## 通用约定

- **Base URL**：`https://fuyao.aicubes.cn`。
- 公开 API 使用 `X-api-key`。
- 期货合约使用完整 `thscode`，品种代码使用大写形式。
- ISO 日期按 `Asia/Shanghai` 解释，Unix 时间戳为毫秒；金融数值与日期来源缺失时可为 `null`，合法无数据时返回 `[]`。

## 期货持仓

来源：https://fuyao.aicubes.cn/docs/api-reference/futures-positions/
...
## 期货品种日持仓 {#positions-variety-daily}

```text
GET /api/futures/positions/variety-daily
```

MCP Tool：[`get_futures_positions_variety_daily`](/docs/mcp/tools/get_futures_positions_variety_daily)
...
## 期货公司品种日持仓 {#positions-company-variety-daily}

```text
GET /api/futures/positions/company-variety-daily
```

MCP Tool：[`get_futures_positions_company_variety_daily`](/docs/mcp/tools/get_futures_positions_company_variety_daily)
...
## 期货公司合约日持仓 {#positions-contract-daily}

```text
GET /api/futures/positions/contract-daily
```

MCP Tool：[`get_futures_positions_contract_daily`](/docs/mcp/tools/get_futures_positions_contract_daily)
...
## 期货公司合约历史持仓 {#positions-contract-historical}

```text
GET /api/futures/positions/contract-historical
```

MCP Tool：[`get_futures_positions_contract_historical`](/docs/mcp/tools/get_futures_positions_contract_historical)
...
字段与[期货公司合约日持仓](#positions-contract-daily)一致；`position_item[]` 与 `average_item[]` 是独立序列。

## 期货公司列表 {#positions-company-list}

```text
GET /api/futures/positions/company-list
```

MCP Tool：[`get_futures_positions_company_list`](/docs/mcp/tools/get_futures_positions_company_list)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-prices/

> 查询期货合约及期货商品指数分时与 K 线行情
...
## 期货分时 {#prices-intraday}

```text
GET /api/futures/prices/intraday
```

MCP Tool：[`get_futures_prices_intraday`](/docs/mcp/tools/get_futures_prices_intraday)
...
MCP Tool：[`get_futures_prices_daily`](/docs/mcp/tools/get_futures_prices_daily)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-reference/
...
## 期货合约详情 {#contracts-detail}

```text
GET /api/futures/contracts/detail
```

MCP Tool：[`get_futures_contracts_detail`](/docs/mcp/tools/get_futures_contracts_detail)
...
## 期货合约基础信息列表 {#contracts-list}

```text
GET /api/futures/contracts/list
```

MCP Tool：[`get_futures_contracts_list`](/docs/mcp/tools/get_futures_contracts_list)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-session-timeline/
...
MCP Tool：[`get_futures_calendar_session_timeline`](/docs/mcp/tools/get_futures_calendar_session_timeline)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-varieties/
...
MCP Tool：[`get_futures_varieties_list`](/docs/mcp/tools/get_futures_varieties_list)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-variety-plates/
...
MCP Tool：[`get_futures_variety_plates_list`](/docs/mcp/tools/get_futures_variety_plates_list)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/futures-warehouse-receipts/
...
MCP Tool：[`get_futures_warehouse_receipts_historical`](/docs/mcp/tools/get_futures_warehouse_receipts_historical)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/high-frequency/

> 查询 A 股个股与指数的高频历史和单日分时数据
...
- **Base URL**：`https://fuyao.aicubes.cn`。
- 本页接口结构用于说明同花顺AI客户端内的数据契约，当前不作为端外接入入口。
- 所有接口返回统一 `ApiResponse` 信封；时间戳为毫秒级 Unix 时间戳，时间与交易日窗口按 `Asia/Shanghai` 判断。
...
来源：https://fuyao.aicubes.cn/docs/api-reference/hot-list-data/
...
响应结构与 [飙升榜](#飙升榜) 一致，`data.item[]` 为 A 股热股榜单股票条目。
...
来源：https://fuyao.aicubes.cn/docs/api-reference/index-overview/

> 指数基本信息、成分股、权重接口（敬请期待）

:::info 敬请期待
**指数概况** 模块正在规划中，将提供宽基指数与同花顺自有指数共用的基础查询能力。
:::
...
来源：https://fuyao.aicubes.cn/docs/api-reference/limit-up-data/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/news-events/

> 仅供同花顺AI客户端内使用的资讯事件检索
...
来源：https://fuyao.aicubes.cn/docs/api-reference/options-overview/

> 期权基础资料、行情与交易时间 API 总览

期权模块提供品种、合约详情、分时、K 线和交易时间轴数据查询。

## 功能分类

| 分类                                             | 路径前缀                                 | 说明                                                                     |
| ------------------------------------------------ | ---------------------------------------- | ------------------------------------------------------------------------ |
| [期权品种资料](./options-varieties.mdx)          | `/api/options/varieties`                 | 查询期权品种目录及交易属性。                                             |
| [期权合约基础资料](./options-reference.mdx)      | `/api/options/contracts`                 | 查询期权合约详情和分页合约目录。                                         |
| [期权行情](./options-prices.mdx)                 | `/api/options/prices`                    | 端外查询当前交易日分时与日 K；AI 客户端内还可查询历史分时与多周期 K 线。 |
| [期权交易时间轴](./options-session-timeline.mdx) | `/api/options/calendar/session-timeline` | 查询期权合约最近交易日的交易时间轴。                                     |
...
- 公开 API 使用 `X-api-key`。
- 期权合约使用完整 `thscode`。
- 日期按 `Asia/Shanghai` 解释，Unix 时间戳为毫秒；金融数值与日期来源缺失时可为 `null`，列表无数据时返回 `[]`。

## 期权行情

来源：https://fuyao.aicubes.cn/docs/api-reference/options-prices/
...
## 期权分时 {#prices-intraday}

```text
GET /api/options/prices/intraday
```

MCP Tool：[`get_options_prices_intraday`](/docs/mcp/tools/get_options_prices_intraday)
...
MCP Tool：[`get_options_prices_daily`](/docs/mcp/tools/get_options_prices_daily)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/options-reference/
...
## 期权合约详情 {#contracts-detail}

```text
GET /api/options/contracts/detail
```

MCP Tool：[`get_options_contracts_detail`](/docs/mcp/tools/get_options_contracts_detail)
...
## 期权合约基础信息列表 {#contracts-list}

```text
GET /api/options/contracts/list
```

MCP Tool：[`get_options_contracts_list`](/docs/mcp/tools/get_options_contracts_list)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/options-session-timeline/
...
MCP Tool：[`get_options_calendar_session_timeline`](/docs/mcp/tools/get_options_calendar_session_timeline)
...
字段结构与[期货交易时间轴](./futures-session-timeline.mdx)一致，使用期权合约代码和最近交易日的交易时间。

## 期权品种资料

来源：https://fuyao.aicubes.cn/docs/api-reference/options-varieties/
...
MCP Tool：[`get_options_varieties_list`](/docs/mcp/tools/get_options_varieties_list)
...
来源：https://fuyao.aicubes.cn/docs/api-reference/stock-basics/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/ths-index-membership/
...
来源：https://fuyao.aicubes.cn/docs/api-reference/valuations/
...
MCP Tool：[`get_a_share_valuations_snapshot`](/docs/mcp/tools/get_a_share_valuations_snapshot)
...
| ----------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------ | ---------------------- | ------------------------------------------------------- |
| fuyao-news-mcp          | `/mcp/news`          | [`get_news_events_search`](./tools/get_news_events_search.mdx)                                                     | 资讯事件库搜索         | `GET /api/news/events/search`                           |
...
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_valuations_snapshot`](./tools/get_a_share_valuations_snapshot.mdx)                                   | A股估值快照            | `GET /api/a-share/valuations/snapshot`                  |
...
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_financials_indicators`](./tools/get_a_share_financials_indicators.mdx)                               | 财务指标数据           | `GET /api/a-share/financials/indicators`                |
...
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_auction_snapshot`](./tools/get_a_share_auction_snapshot.mdx)                                         | A股集合竞价快照        | `GET /api/a-share/auction/snapshot`                     |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_auction_short_term_benchmark`](./tools/get_a_share_auction_short_term_benchmark.mdx)                 | 短线风向标竞价基准     | `GET /api/a-share/auction/short-term-benchmark`         |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_special_data_limit_up_pool`](./tools/get_a_share_special_data_limit_up_pool.mdx)                     | A股涨停池              | `GET /api/a-share/special-data/limit-up-pool`           |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_special_data_limit_down_pool`](./tools/get_a_share_special_data_limit_down_pool.mdx)                 | A股跌停池              | `GET /api/a-share/special-data/limit-down-pool`         |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_special_data_limit_break_pool`](./tools/get_a_share_special_data_limit_break_pool.mdx)               | A股炸板池              | `GET /api/a-share/special-data/limit-break-pool`        |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_special_data_limit_up_ladder`](./tools/get_a_share_special_data_limit_up_ladder.mdx)                 | A股涨停天梯            | `GET /api/a-share/special-data/limit-up-ladder`         |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_special_data_skyrocket_list`](./tools/get_a_share_special_data_skyrocket_list.mdx)                   | A股飙升榜              | `GET /api/a-share/special-data/skyrocket-list`          |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_special_data_hot_stock_list`](./tools/get_a_share_special_data_hot_stock_list.mdx)                   | A股热股榜              | `GET /api/a-share/special-data/hot-stock-list`          |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_special_data_hot_stock_list_history`](./tools/get_a_share_special_data_hot_stock_list_history.mdx)   | A股历史热股榜          | `GET /api/a-share/special-data/hot-stock-list-history`  |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_special_data_hot_stock_rank_trend`](./tools/get_a_share_special_data_hot_stock_rank_trend.mdx)       | A股热股排名趋势        | `GET /api/a-share/special-data/hot-stock-rank-trend`    |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_special_data_dragon_tiger_list`](./tools/get_a_share_special_data_dragon_tiger_list.mdx)             | A股龙虎榜              | `GET /api/a-share/special-data/dragon-tiger-list`       |
| fuyao-a-share-mcp       | `/mcp/a-share`       | [`get_a_share_special_data_anomaly_analysis_stock`](./tools/get_a_share_special_data_anomaly_analysis_stock.mdx)   | A股个股异动原因        | `GET /api/a-share/special-data/anomaly-analysis-stock`  |
| fuyao-a-share-index-mcp | `/mcp/a-share-index` | [`get_a_share_index_catalog_ths_index_list`](./tools/get_a_share_index_catalog_ths_index_list.mdx)                 | THS 指数目录           | `GET /api/a-share-index/catalog/ths-index-list`         |
| fuyao-a-share-index-mcp | `/mcp/a-share-index` | [`get_a_share_index_constituents_ths_stock_list`](./tools/get_a_share_index_constituents_ths_stock_list.mdx)       | THS 指数成分股列表     | `GET /api/a-share-index/constituents/ths-stock-list`    |
| fuyao-a-share-index-mcp | `/mcp/a-share-index` | [`get_a_share_index_prices_snapshot`](./tools/get_a_share_index_prices_snapshot.mdx)                               | A股指数行情快照        | `GET /api/a-share-index/prices/snapshot`                |
| fuyao-a-share-index-mcp | `/mcp/a-share-index` | [`get_a_share_index_prices_historical`](./tools/get_a_share_index_prices_historical.mdx)                           | A股指数历史K线         | `GET /api/a-share-index/prices/historical`              |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_varieties_list`](./tools/get_futures_varieties_list.mdx)                                             | 期货品种资料           | `GET /api/futures/varieties/list`                       |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_contracts_detail`](./tools/get_futures_contracts_detail.mdx)                                         | 期货合约详情           | `GET /api/futures/contracts/detail`                     |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_contracts_list`](./tools/get_futures_contracts_list.mdx)                                             | 期货合约基础信息列表   | `GET /api/futures/contracts/list`                       |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_variety_plates_list`](./tools/get_futures_variety_plates_list.mdx)                                   | 期货品种板块           | `GET /api/futures/variety-plates/list`                  |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_contracts_main_continuous_list`](./tools/get_futures_contracts_main_continuous_list.mdx)             | 期货主连列表           | `GET /api/futures/contracts/main-continuous-list`       |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_contracts_main_list`](./tools/get_futures_contracts_main_list.mdx)                                   | 期货主力合约列表       | `GET /api/futures/contracts/main-list`                  |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_contracts_secondary_main_list`](./tools/get_futures_contracts_secondary_main_list.mdx)               | 期货次主力合约列表     | `GET /api/futures/contracts/secondary-main-list`        |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_contracts_commodity_index_list`](./tools/get_futures_contracts_commodity_index_list.mdx)             | 期货商品指数资料       | `GET /api/futures/contracts/commodity-index-list`       |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_positions_variety_daily`](./tools/get_futures_positions_variety_daily.mdx)                           | 期货品种日持仓         | `GET /api/futures/positions/variety-daily`              |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_positions_company_variety_daily`](./tools/get_futures_positions_company_variety_daily.mdx)           | 公司品种日持仓         | `GET /api/futures/positions/company-variety-daily`      |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_positions_contract_daily`](./tools/get_futures_positions_contract_daily.mdx)                         | 期货合约日持仓         | `GET /api/futures/positions/contract-daily`             |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_positions_contract_historical`](./tools/get_futures_positions_contract_historical.mdx)               | 期货合约历史持仓       | `GET /api/futures/positions/contract-historical`        |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_positions_company_list`](./tools/get_futures_positions_company_list.mdx)                             | 期货公司列表           | `GET /api/futures/positions/company-list`               |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_warehouse_receipts_historical`](./tools/get_futures_warehouse_receipts_historical.mdx)               | 期货历史仓单           | `GET /api/futures/warehouse-receipts/historical`        |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_basis_main_continuous_latest`](./tools/get_futures_basis_main_continuous_latest.mdx)                 | 期货主连最新基差       | `GET /api/futures/basis/main-continuous-latest`         |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_basis_historical`](./tools/get_futures_basis_historical.mdx)                                         | 期货历史基差           | `GET /api/futures/basis/historical`                     |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_calendar_trading_schedule`](./tools/get_futures_calendar_trading_schedule.mdx)                       | 期货交易日日程         | `GET /api/futures/calendar/trading-schedule`            |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_calendar_session_timeline`](./tools/get_futures_calendar_session_timeline.mdx)                       | 期货会话时间轴         | `GET /api/futures/calendar/session-timeline`            |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_prices_intraday`](./tools/get_futures_prices_intraday.mdx)                                           | 期货分时               | `GET /api/futures/prices/intraday`                      |
| fuyao-futures-mcp       | `/mcp/futures`       | [`get_futures_prices_daily`](./tools/get_futures_prices_daily.mdx)                                                 | 期货K线                | `GET /api/futures/prices/daily`                         |
| fuyao-options-mcp       | `/mcp/options`       | [`get_options_varieties_list`](./tools/get_options_varieties_list.mdx)                                             | 期权品种资料           | `GET /api/options/varieties/list`                       |
| fuyao-options-mcp       | `/mcp/options`       | [`get_options_contracts_detail`](./tools/get_options_contracts_detail.mdx)                                         | 期权合约详情           | `GET /api/options/contracts/detail`                     |
| fuyao-options-mcp       | `/mcp/options`       | [`get_options_contracts_list`](./tools/get_options_contracts_list.mdx)                                             | 期权合约基础信息列表   | `GET /api/options/contracts/list`                       |
| fuyao-options-mcp       | `/mcp/options`       | [`get_options_calendar_session_timeline`](./tools/get_options_calendar_session_timeline.mdx)                       | 期权会话时间轴         | `GET /api/options/calendar/session-timeline`            |
| fuyao-options-mcp       | `/mcp/options`       | [`get_options_prices_intraday`](./tools/get_options_prices_intraday.mdx)                                           | 期权分时               | `GET /api/options/prices/intraday`                      |
| fuyao-options-mcp       | `/mcp/options`       | [`get_options_prices_daily`](./tools/get_options_prices_daily.mdx)                                                 | 期权K线                | `GET /api/options/prices/daily`                         |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_profile_detail`](./tools/get_fund_profile_detail.mdx)                                                   | 基金基本资料           | `GET /api/fund/profile/detail`                          |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_portfolio_holdings`](./tools/get_fund_portfolio_holdings.mdx)                                           | 基金重仓股             | `GET /api/fund/portfolio/holdings`                      |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_performance_nav`](./tools/get_fund_performance_nav.mdx)                                                 | 基金净值               | `GET /api/fund/performance/nav`                         |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_performance_returns`](./tools/get_fund_performance_returns.mdx)                                         | 基金区间收益           | `GET /api/fund/performance/returns`                     |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_holders_detail`](./tools/get_fund_holders_detail.mdx)                                                   | 基金持有人结构         | `GET /api/fund/holders/detail`                          |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_market_snapshot`](./tools/get_fund_market_snapshot.mdx)                                                 | 基金行情快照           | `GET /api/fund/market/snapshot`                         |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_market_historical`](./tools/get_fund_market_historical.mdx)                                             | ETF 历史日线行情       | `GET /api/fund/market/historical`                       |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_companies_detail`](./tools/get_fund_companies_detail.mdx)                                               | 基金公司详情           | `GET /api/fund/companies/detail`                        |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_portfolio_industry_allocation`](./tools/get_fund_portfolio_industry_allocation.mdx)                     | 基金行业配置           | `GET /api/fund/portfolio/industry-allocation`           |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_performance_indicators_historical`](./tools/get_fund_performance_indicators_historical.mdx)             | 基金历史业绩指标       | `GET /api/fund/performance/indicators-historical`       |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_performance_drawdowns`](./tools/get_fund_performance_drawdowns.mdx)                                     | 基金回撤指标           | `GET /api/fund/performance/drawdowns`                   |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_holders_top`](./tools/get_fund_holders_top.mdx)                                                         | 基金前十大持有人       | `GET /api/fund/holders/top`                             |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_corporate_actions_dividends`](./tools/get_fund_corporate_actions_dividends.mdx)                         | 基金分红记录           | `GET /api/fund/corporate-actions/dividends`             |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_diagnostics_detail`](./tools/get_fund_diagnostics_detail.mdx)                                           | 基金诊断详情           | `GET /api/fund/diagnostics/detail`                      |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_financials_indicators`](./tools/get_fund_financials_indicators.mdx)                                     | 基金财务指标           | `GET /api/fund/financials/indicators`                   |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_financials_income_statements`](./tools/get_fund_financials_income_statements.mdx)                       | 基金利润表             | `GET /api/fund/financials/income-statements`            |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_financials_balance_sheets`](./tools/get_fund_financials_balance_sheets.mdx)                             | 基金资产负债表         | `GET /api/fund/financials/balance-sheets`               |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_backtest_result`](./tools/get_fund_backtest_result.mdx)                                                 | 基金在线回测           | `GET /api/fund/backtest/result`                         |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_backtest_indicators`](./tools/get_fund_backtest_indicators.mdx)                                         | 基金回测指标           | `GET /api/fund/backtest/indicators`                     |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_indicators_line`](./tools/get_fund_indicators_line.mdx)                                                 | 基金画线指标           | `GET /api/fund/indicators/line`                         |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_indicators_table`](./tools/get_fund_indicators_table.mdx)                                               | 基金表格指标           | `GET /api/fund/indicators/table`                        |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_quota_summary`](./tools/get_fund_quota_summary.mdx)                                                     | QDII额度汇总           | `GET /api/fund/quota/summary`                           |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_quota_list`](./tools/get_fund_quota_list.mdx)                                                           | QDII额度列表           | `GET /api/fund/quota/list`                              |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_managers_investment_style`](./tools/get_fund_managers_investment_style.mdx)                             | 基金经理投资风格       | `GET /api/fund/managers/investment-style`               |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_managers_performance`](./tools/get_fund_managers_performance.mdx)                                       | 基金经理业绩           | `GET /api/fund/managers/performance`                    |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_managers_experience`](./tools/get_fund_managers_experience.mdx)                                         | 基金经理从业经历       | `GET /api/fund/managers/experience`                     |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_managers_detail`](./tools/get_fund_managers_detail.mdx)                                                 | 基金经理详情           | `GET /api/fund/managers/detail`                         |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_news_article_list`](./tools/get_fund_news_article_list.mdx)                                             | 基金资讯列表           | `GET /api/fund/news/article-list`                       |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_offerings_list`](./tools/get_fund_offerings_list.mdx)                                                   | 基金募集列表           | `GET /api/fund/offerings/list`                          |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_portfolio_stock_history`](./tools/get_fund_portfolio_stock_history.mdx)                                 | 基金历史股票持仓       | `GET /api/fund/portfolio/stock-history`                 |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_portfolio_stock_report_dates`](./tools/get_fund_portfolio_stock_report_dates.mdx)                       | 基金股票持仓报告日期   | `GET /api/fund/portfolio/stock-report-dates`            |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_portfolio_bond_history`](./tools/get_fund_portfolio_bond_history.mdx)                                   | 基金历史债券持仓       | `GET /api/fund/portfolio/bond-history`                  |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_portfolio_bond_report_dates`](./tools/get_fund_portfolio_bond_report_dates.mdx)                         | 基金债券持仓报告日期   | `GET /api/fund/portfolio/bond-report-dates`             |
| fuyao-fund-mcp          | `/mcp/fund`          | [`get_fund_portfolio_asset_allocation`](./tools/get_fund_portfolio_asset_allocation.mdx)                           | 基金资产配置           | `GET /api/fund/portfolio/asset-allocation`              |
...
1. `fuyao-meta-mcp` → `get_meta_tickers_list`，`asset_type: "a-share"`，`limit: 1000`，`offset: 0`
...
1. 如不确定代码，先用 `fuyao-meta-mcp` → `get_meta_tickers_search` 分别检索 "上证综指" / "白酒概念"
2. `fuyao-a-share-index-mcp` → `get_a_share_index_prices_snapshot`，
   `thscodes: "000001.SH,886042.TI"`，批量取指数快照

### 场景十：获取指数历史走势

用户："白酒概念最近一年的日 K 线"

1. `fuyao-meta-mcp` → `get_meta_tickers_search`，`q: "白酒概念"`，获取 `thscode: 886042.TI`
2. 计算当前与一年前的毫秒时间戳
3. `fuyao-a-share-index-mcp` → `get_a_share_index_prices_historical`，
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_auction_short_term_benchmark/
...
对应 REST 端点：[`GET /api/a-share/auction/short-term-benchmark`](/docs/api-reference/auction#短线风向标竞价基准)
:::
...
返回 `{ timestamp, date, date_ms, item[] }`。`timestamp` 是接口响应组装时间，`date` / `date_ms` 是最终查询日期；明细包含标准代码、名称、竞价涨跌幅和标签。字段见 [短线风向标竞价基准](/docs/api-reference/auction#短线风向标竞价基准)。

## A股集合竞价快照

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_auction_snapshot/
...
对应 REST 端点：[`GET /api/a-share/auction/snapshot`](/docs/api-reference/auction#a股集合竞价快照)
:::
...
返回接口响应组装时间、集合竞价阶段、数据状态、总数及竞价明细；字段见 [A股集合竞价快照](/docs/api-reference/auction#a股集合竞价快照)。
...
字段含义见 REST 端点 [资产负债表返回字段](/docs/api-reference/financials#balance-sheets-return-fields) 与
...
字段含义见 REST 端点 [现金流量表返回字段](/docs/api-reference/financials#cash-flow-statements-return-fields) 与
[共有响应字段](/docs/api-reference/financials#共有响应字段)。
...
字段含义见 REST 端点 [利润表返回字段](/docs/api-reference/financials#income-statements-return-fields) 与
[共有响应字段](/docs/api-reference/financials#共有响应字段)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_financials_indicators/
...
对应 REST 端点：[`GET /api/a-share/financials/indicators`](/docs/api-reference/financial-indicators)
:::
...
字段含义见 REST 端点 [财务指标数据](/docs/api-reference/financial-indicators)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_index_overview/

> 指数基本信息、成分股、权重 MCP 工具（敬请期待）
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_index_prices_historical/

> 单只 A 股指数、同花顺板块或行业指数的历史 K 线序列

:::info
工具名：`get_a_share_index_prices_historical`

对应 REST 端点：[`GET /api/a-share-index/prices/historical`](/docs/api-reference/a-share-index#指数历史-k-线)
:::
...
[指数历史 K 线](/docs/api-reference/a-share-index#指数历史-k-线)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_index_prices_snapshot/

> 按 thscodes 批量获取 A 股指数、同花顺板块或行业指数的最新行情

:::info
工具名：`get_a_share_index_prices_snapshot`

对应 REST 端点：[`GET /api/a-share-index/prices/snapshot`](/docs/api-reference/a-share-index#指数行情快照)
:::
...
[指数行情快照](/docs/api-reference/a-share-index#指数行情快照)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_special_data_anomaly_analysis_stock/
...
对应 REST 端点：[`GET /api/a-share/special-data/anomaly-analysis-stock`](/docs/api-reference/anomaly-analysis#按股票查询个股异动原因)
:::
...
字段含义见 REST 端点 [按股票查询个股异动原因](/docs/api-reference/anomaly-analysis#按股票查询个股异动原因)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_special_data_dragon_tiger_list/

> 查询龙虎榜榜单，覆盖全部、机构榜与游资榜

:::info
工具名：`get_a_share_special_data_dragon_tiger_list`

对应 REST 端点：[`GET /api/a-share/special-data/dragon-tiger-list`](/docs/api-reference/dragon-tiger-data#龙虎榜榜单)
:::
...
字段含义见 REST 端点 [龙虎榜榜单](/docs/api-reference/dragon-tiger-data#龙虎榜榜单)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_special_data_hot_stock_list_history/
...
对应 REST 端点：[`GET /api/a-share/special-data/hot-stock-list-history`](/docs/api-reference/hot-list-data#历史热股排行)
:::
...
不会在每个 `item` 内重复出现。字段含义见 REST 端点 [历史热股排行](/docs/api-reference/hot-list-data#历史热股排行)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_special_data_hot_stock_list/
...
对应 REST 端点：[`GET /api/a-share/special-data/hot-stock-list`](/docs/api-reference/hot-list-data#a股热股榜单)
:::
...
返回 `{ timestamp, item: [HotListItem, ...] }`。字段含义见 REST 端点 [A股热股榜单](/docs/api-reference/hot-list-data#a股热股榜单)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_special_data_hot_stock_rank_trend/

> 查询单只 A 股在指定日期区间内的热股榜排名走势

:::info
工具名：`get_a_share_special_data_hot_stock_rank_trend`

对应 REST 端点：[`GET /api/a-share/special-data/hot-stock-rank-trend`](/docs/api-reference/hot-list-data#个股排名走势)
:::
...
返回 `{ timestamp, item: [HotListRankTrendItem, ...] }`。字段含义见 REST 端点 [个股排名走势](/docs/api-reference/hot-list-data#个股排名走势)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_special_data_limit_break_pool/
...
对应 REST 端点：[`GET /api/a-share/special-data/limit-break-pool`](/docs/api-reference/limit-up-data#炸板股票池)
:::
...
返回 `{ timestamp, pagination, item[] }`；字段见 [炸板股票池](/docs/api-reference/limit-up-data#炸板股票池)。

## A股跌停池

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_special_data_limit_down_pool/
...
对应 REST 端点：[`GET /api/a-share/special-data/limit-down-pool`](/docs/api-reference/limit-up-data#跌停股票池)
:::
...
返回 `{ timestamp, pagination, item[] }`；字段见 [跌停股票池](/docs/api-reference/limit-up-data#跌停股票池)。

## A股涨停天梯

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_special_data_limit_up_ladder/
...
对应 REST 端点：[`GET /api/a-share/special-data/limit-up-ladder`](/docs/api-reference/limit-up-data#连板天梯)
:::
...
字段含义见 REST 端点 [连板天梯](/docs/api-reference/limit-up-data#响应字段-1)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_special_data_limit_up_pool/
...
对应 REST 端点：[`GET /api/a-share/special-data/limit-up-pool`](/docs/api-reference/limit-up-data#涨停股票池)
:::
...
字段含义见 REST 端点 [涨停股票池](/docs/api-reference/limit-up-data#响应字段)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_special_data_skyrocket_list/
...
对应 REST 端点：[`GET /api/a-share/special-data/skyrocket-list`](/docs/api-reference/hot-list-data#飙升榜)
:::
...
返回 `{ timestamp, item: [HotListItem, ...] }`。字段含义见 REST 端点 [飙升榜](/docs/api-reference/hot-list-data#飙升榜)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_stock_basics/

> A 股标的基础信息查询 MCP 工具（敬请期待）
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_ths_index_membership/

> 按个股反查所属同花顺行业/概念指数 MCP 工具（敬请期待）

:::info 敬请期待
工具名：`get_a_share_ths_index_membership`

对应 MCP 工具正在规划中，将按个股 thscode 反查其所属的同花顺行业、概念等指数集合。
:::

## A股估值快照

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_a_share_valuations_snapshot/
...
对应 REST 端点：[`GET /api/a-share/valuations/snapshot`](/docs/api-reference/valuations)
:::
...
[估值数据](/docs/api-reference/valuations)。

## 基金回测指标

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_backtest_indicators/
...
对应 REST 端点：[`GET /api/fund/backtest/indicators`](/docs/api-reference/fund-backtest#基金回测可用指标)
:::
...
```text
工具：get_fund_backtest_indicators
参数：{}
```

## 返回

返回指标编码、名称、操作符和状态规则；详见[基金回测可用指标](/docs/api-reference/fund-backtest#基金回测可用指标)。

## 基金在线回测

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_backtest_result/
...
对应 REST 端点：[`GET /api/fund/backtest/result`](/docs/api-reference/fund-backtest#基金在线回测)
:::
...
返回交易明细、回测指标与收益曲线；详见[基金在线回测](/docs/api-reference/fund-backtest#基金在线回测)。

## 基金公司详情

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_companies_detail/
...
对应 REST 端点：[`GET /api/fund/companies/detail`](/docs/api-reference/fund-company)
:::
...
返回 `{ timestamp, item[] }`；字段见 [基金公司详情](/docs/api-reference/fund-company)。

## 基金分红记录

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_corporate_actions_dividends/
...
对应 REST 端点：[`GET /api/fund/corporate-actions/dividends`](/docs/api-reference/fund-corporate-actions)
:::
...
返回现金分红、进度和关键日期；字段见 [基金分红记录](/docs/api-reference/fund-corporate-actions)。

## 基金诊断详情

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_diagnostics_detail/
...
对应 REST 端点：[`GET /api/fund/diagnostics/detail`](/docs/api-reference/fund-diagnostics)
:::
...
返回诊断维度、同类维度、概率、区间和韧性结构；字段见 [基金诊断详情](/docs/api-reference/fund-diagnostics)。

## 基金资产负债表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_financials_balance_sheets/
...
对应 REST 端点：[`GET /api/fund/financials/balance-sheets`](/docs/api-reference/fund-financials#基金资产负债表)
:::
...
返回报告期、资产、负债与所有者权益字段；字段见 [基金资产负债表](/docs/api-reference/fund-financials#基金资产负债表)。

## 基金利润表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_financials_income_statements/
...
对应 REST 端点：[`GET /api/fund/financials/income-statements`](/docs/api-reference/fund-financials#基金利润表)
:::
...
返回报告期、收入、费用与利润字段；字段见 [基金利润表](/docs/api-reference/fund-financials#基金利润表)。

## 基金财务指标

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_financials_indicators/
...
对应 REST 端点：[`GET /api/fund/financials/indicators`](/docs/api-reference/fund-financials#基金财务指标)
:::
...
返回报告期及基金主要财务指标；字段见 [基金财务指标](/docs/api-reference/fund-financials#基金财务指标)。

## 基金持有人结构

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_holders_detail/
...
对应 REST 端点：[`GET /api/fund/holders/detail`](/docs/api-reference/fund-holders)
:::
...
[基金持有人结构](/docs/api-reference/fund-holders#返回字段)。

## 基金前十大持有人

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_holders_top/
...
对应 REST 端点：[`GET /api/fund/holders/top`](/docs/api-reference/fund-holders#基金前十大持有人)
:::
...
返回持有人、排名、份额、比例与报告日期；字段见 [基金前十大持有人](/docs/api-reference/fund-holders#基金前十大持有人)。

## 基金画线指标

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_indicators_line/
...
对应 REST 端点：[`GET /api/fund/indicators/line`](/docs/api-reference/fund-indicators#基金画线指标)
:::
...
返回 Unix 毫秒时间轴、指标元信息、完整 `thscode` 和数值数组；详见[基金画线指标](/docs/api-reference/fund-indicators#基金画线指标)。

## 基金表格指标

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_indicators_table/
...
对应 REST 端点：[`GET /api/fund/indicators/table`](/docs/api-reference/fund-indicators#基金表格指标)
:::
...
返回总数、指标元信息、完整 `thscode`、指标值及 `part_order_thscodes`；详见[基金表格指标](/docs/api-reference/fund-indicators#基金表格指标)。

## 基金经理详情

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_managers_detail/
...
对应 REST 端点：[`GET /api/fund/managers/detail`](/docs/api-reference/fund-managers#基金经理详情)
:::
...
返回经理资料、公司、收益与雷达对比；字段见 [基金经理详情](/docs/api-reference/fund-managers#基金经理详情)。

## 基金经理从业经历

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_managers_experience/
...
对应 REST 端点：[`GET /api/fund/managers/experience`](/docs/api-reference/fund-managers#从业经历)
:::
...
返回荣誉、重仓资产与投资经历三组结构化数据；字段见 [从业经历](/docs/api-reference/fund-managers#从业经历)。

## 基金经理投资风格

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_managers_investment_style/
...
对应 REST 端点：[`GET /api/fund/managers/investment-style`](/docs/api-reference/fund-managers#投资风格)
:::
...
返回代表基金、投资理念、管理规模与行业偏好；字段见 [投资风格](/docs/api-reference/fund-managers#投资风格)。

## 基金经理业绩

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_managers_performance/
...
对应 REST 端点：[`GET /api/fund/managers/performance`](/docs/api-reference/fund-managers#基金经理业绩)
:::
...
返回经理、同类和基准的收益序列；字段见 [基金经理业绩](/docs/api-reference/fund-managers#基金经理业绩)。

## ETF 历史日线行情

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_market_historical/

> 查询单只场内基金近 5 年内的前复权日线行情（当前仅支持 ETF）

:::info
工具名：`get_fund_market_historical`

对应 REST 端点：[`GET /api/fund/market/historical`](/docs/api-reference/fund-market#场内基金历史日线行情)
:::
...
[场内基金历史日线行情](/docs/api-reference/fund-market#场内基金历史日线行情)。

## 基金行情快照

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_market_snapshot/

> 查询 ETF 和 LOF 的最新行情快照

:::info
工具名：`get_fund_market_snapshot`

对应 REST 端点：[`GET /api/fund/market/snapshot`](/docs/api-reference/fund-market#场内基金行情快照)
:::
...
[场内基金行情快照](/docs/api-reference/fund-market#场内基金行情快照)。

## 基金资讯列表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_news_article_list/
...
对应 REST 端点：[`GET /api/fund/news/article-list`](/docs/api-reference/fund-news)
:::
...
返回 `{ timestamp, limit, offset, has_more, item[] }`，不包含 `total`；是否继续翻页以 `has_more` 为准。字段见 [基金资讯列表](/docs/api-reference/fund-news)。

## 基金募集列表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_offerings_list/
...
对应 REST 端点：[`GET /api/fund/offerings/list`](/docs/api-reference/fund-offerings)
:::
...
返回基金代码与募集起止时间；字段见 [基金募集列表](/docs/api-reference/fund-offerings)。

## 基金回撤指标

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_performance_drawdowns/
...
对应 REST 端点：[`GET /api/fund/performance/drawdowns`](/docs/api-reference/fund-performance#基金最大回撤)
:::
...
返回基金代码与十个区间最大回撤；字段见 [基金最大回撤](/docs/api-reference/fund-performance#基金最大回撤)。

## 基金历史业绩指标

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_performance_indicators_historical/
...
对应 REST 端点：[`GET /api/fund/performance/indicators-historical`](/docs/api-reference/fund-performance#基金历史业绩指标)
:::
...
返回 `{ timestamp, item[] }`，`timestamp` 保留明确的上游数据时间；固定周期 `DAY_1` 不作为顶层字段返回，也不返回顶层 `thscode` 或 `interval`。字段见 [基金历史业绩指标](/docs/api-reference/fund-performance#基金历史业绩指标)。

## 基金净值

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_performance_nav/

> 查询基金单位净值、复权净值与区间净值序列

:::info
工具名：`get_fund_performance_nav`

对应 REST 端点：[`GET /api/fund/performance/nav`](/docs/api-reference/fund-performance#基金净值)
:::
...
[基金净值](/docs/api-reference/fund-performance#基金净值)。

## 基金区间收益

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_performance_returns/
...
对应 REST 端点：[`GET /api/fund/performance/returns`](/docs/api-reference/fund-performance#基金区间收益)
:::
...
[基金区间收益](/docs/api-reference/fund-performance#基金区间收益)。

## 基金资产配置

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_portfolio_asset_allocation/
...
对应 REST 端点：[`GET /api/fund/portfolio/asset-allocation`](/docs/api-reference/fund-portfolio#基金资产配置)
:::
...
返回报告日期与各资产配置比例；字段见 [基金资产配置](/docs/api-reference/fund-portfolio#基金资产配置)。

## 基金历史债券持仓

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_portfolio_bond_history/
...
对应 REST 端点：[`GET /api/fund/portfolio/bond-history`](/docs/api-reference/fund-portfolio#基金历史债券持仓)
:::
...
返回历史债券持仓明细；`rank` 仅前十大持仓为 `1`–`10`，其余持仓为 `null`。字段见 [基金历史债券持仓](/docs/api-reference/fund-portfolio#基金历史债券持仓)。

## 基金债券持仓报告日期

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_portfolio_bond_report_dates/
...
对应 REST 端点：[`GET /api/fund/portfolio/bond-report-dates`](/docs/api-reference/fund-portfolio#基金债券持仓报告日期)
:::
...
返回报告类型、名称与起止日期；字段见 [基金债券持仓报告日期](/docs/api-reference/fund-portfolio#基金债券持仓报告日期)。

## 基金重仓股

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_portfolio_holdings/
...
对应 REST 端点：[`GET /api/fund/portfolio/holdings`](/docs/api-reference/fund-holdings)
:::
...
[基金重仓股](/docs/api-reference/fund-holdings#返回字段)。

## 基金行业配置

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_portfolio_industry_allocation/
...
对应 REST 端点：[`GET /api/fund/portfolio/industry-allocation`](/docs/api-reference/fund-portfolio#基金行业配置)
:::
...
返回报告期、行业名称与配置比例；字段见 [基金行业配置](/docs/api-reference/fund-portfolio#基金行业配置)。

## 基金历史股票持仓

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_portfolio_stock_history/

> 查询基金指定报告期的历史股票持仓

:::info
工具名：`get_fund_portfolio_stock_history`

对应 REST 端点：[`GET /api/fund/portfolio/stock-history`](/docs/api-reference/fund-portfolio#基金历史股票持仓)
:::
...
```text
工具：get_fund_portfolio_stock_history
参数：
  - thscode: "510300.SH"
  - report_type: "quarter"
  - end_date: "2026-06-30"
...
返回历史股票持仓明细；`rank` 仅前十大持仓为 `1`–`10`，其余持仓为 `null`。字段见 [基金历史股票持仓](/docs/api-reference/fund-portfolio#基金历史股票持仓)。

## 基金股票持仓报告日期

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_portfolio_stock_report_dates/
...
对应 REST 端点：[`GET /api/fund/portfolio/stock-report-dates`](/docs/api-reference/fund-portfolio#基金股票持仓报告日期)
:::
...
返回报告类型、名称与起止日期；字段见 [基金股票持仓报告日期](/docs/api-reference/fund-portfolio#基金股票持仓报告日期)。

## 基金基本资料

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_profile_detail/
...
对应 REST 端点：[`GET /api/fund/profile/detail`](/docs/api-reference/fund-profile)
:::
...
[基金基本资料](/docs/api-reference/fund-profile#返回字段)。

## QDII额度列表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_quota_list/
...
对应 REST 端点：[`GET /api/fund/quota/list`](/docs/api-reference/fund-quota#qdii额度列表)
:::
...
返回分类下的完整基金 `thscode`、名称、额度、近一年收益率和可空 `classify` 类别列表；详见[QDII额度列表](/docs/api-reference/fund-quota#qdii额度列表)。

## QDII额度汇总

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_fund_quota_summary/
...
对应 REST 端点：[`GET /api/fund/quota/summary`](/docs/api-reference/fund-quota#qdii额度汇总)
:::
...
返回分类额度汇总；详见[QDII额度汇总](/docs/api-reference/fund-quota#qdii额度汇总)。

## 期货历史基差

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_basis_historical/
...
对应 REST 端点：[`GET /api/futures/basis/historical`](/docs/api-reference/futures-basis#basis-historical)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货历史基差](/docs/api-reference/futures-basis#basis-historical)。

## 期货主连最新基差

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_basis_main_continuous_latest/

> 查询期货主连的最新现货、期货价格与基差。

:::info
工具名：`get_futures_basis_main_continuous_latest`

对应 REST 端点：[`GET /api/futures/basis/main-continuous-latest`](/docs/api-reference/futures-basis#basis-main-continuous-latest)
:::
...
```text
工具：get_futures_basis_main_continuous_latest
参数：
  无
```
...
返回统一数据对象；字段、空值和数组语义见 [期货主连最新基差](/docs/api-reference/futures-basis#basis-main-continuous-latest)。

## 期货会话时间轴

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_calendar_session_timeline/
...
对应 REST 端点：[`GET /api/futures/calendar/session-timeline`](/docs/api-reference/futures-session-timeline)
:::
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_calendar_trading_schedule/

> 查询指定期货合约在日期范围内的交易日及对应交易时间安排。

:::info
工具名：`get_futures_calendar_trading_schedule`

对应 REST 端点：[`GET /api/futures/calendar/trading-schedule`](/docs/api-reference/futures-calendar)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货交易日](/docs/api-reference/futures-calendar)。

## 期货商品指数资料

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_contracts_commodity_index_list/
...
对应 REST 端点：[`GET /api/futures/contracts/commodity-index-list`](/docs/api-reference/futures-contracts-extended#futures-commodity-indexes)
:::
...
```text
工具：get_futures_contracts_commodity_index_list
参数：
  {}
```
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_contracts_detail/
...
对应 REST 端点：[`GET /api/futures/contracts/detail`](/docs/api-reference/futures-reference#contracts-detail)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货合约详情](/docs/api-reference/futures-reference#contracts-detail)。

## 期货合约基础信息列表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_contracts_list/
...
对应 REST 端点：[`GET /api/futures/contracts/list`](/docs/api-reference/futures-reference#contracts-list)
:::
...
返回期货合约基础信息分页列表，字段详见 [期货合约基础资料](/docs/api-reference/futures-reference#contracts-list)。

## 期货主连列表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_contracts_main_continuous_list/
...
对应 REST 端点：[`GET /api/futures/contracts/main-continuous-list`](/docs/api-reference/futures-contracts-extended#futures-main-continuous)
:::
...
```text
工具：get_futures_contracts_main_continuous_list
参数：
  {}
```
...
返回期货主连列表，字段详见 [期货主力与指数合约](/docs/api-reference/futures-contracts-extended#futures-main-continuous)。

## 期货主力合约列表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_contracts_main_list/

> 查询期货主力合约

:::info
工具名：`get_futures_contracts_main_list`

对应 REST 端点：[`GET /api/futures/contracts/main-list`](/docs/api-reference/futures-contracts-extended#futures-main-contracts)
:::
...
```text
工具：get_futures_contracts_main_list
参数：
  {}
```
...
返回期货主力合约列表，字段详见 [期货主力与指数合约](/docs/api-reference/futures-contracts-extended#futures-main-contracts)。

## 期货次主力合约列表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_contracts_secondary_main_list/

> 查询期货次主力合约

:::info
工具名：`get_futures_contracts_secondary_main_list`

对应 REST 端点：[`GET /api/futures/contracts/secondary-main-list`](/docs/api-reference/futures-contracts-extended#futures-secondary-main-contracts)
:::
...
```text
工具：get_futures_contracts_secondary_main_list
参数：
  {}
```
...
返回期货次主力合约列表，字段详见 [期货主力与指数合约](/docs/api-reference/futures-contracts-extended#futures-secondary-main-contracts)。

## 期货公司列表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_positions_company_list/

> 查询可用于持仓检索的期货公司列表。

:::info
工具名：`get_futures_positions_company_list`

对应 REST 端点：[`GET /api/futures/positions/company-list`](/docs/api-reference/futures-positions#positions-company-list)
:::
...
```text
工具：get_futures_positions_company_list
参数：
  无
```
...
返回统一数据对象；字段、空值和数组语义见 [期货公司列表](/docs/api-reference/futures-positions#positions-company-list)。

## 公司品种日持仓

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_positions_company_variety_daily/
...
对应 REST 端点：[`GET /api/futures/positions/company-variety-daily`](/docs/api-reference/futures-positions#positions-company-variety-daily)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货公司品种日持仓](/docs/api-reference/futures-positions#positions-company-variety-daily)。

## 期货合约日持仓

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_positions_contract_daily/
...
对应 REST 端点：[`GET /api/futures/positions/contract-daily`](/docs/api-reference/futures-positions#positions-contract-daily)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货公司合约日持仓](/docs/api-reference/futures-positions#positions-contract-daily)。

## 期货合约历史持仓

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_positions_contract_historical/
...
对应 REST 端点：[`GET /api/futures/positions/contract-historical`](/docs/api-reference/futures-positions#positions-contract-historical)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货公司合约历史持仓](/docs/api-reference/futures-positions#positions-contract-historical)。

## 期货品种日持仓

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_positions_variety_daily/
...
对应 REST 端点：[`GET /api/futures/positions/variety-daily`](/docs/api-reference/futures-positions#positions-variety-daily)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货品种日持仓](/docs/api-reference/futures-positions#positions-variety-daily)。

## 期货K线

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_prices_daily/

> 查询期货 K 线；端外仅支持日 K，AI 客户端内支持完整周期。

:::info
工具名：`get_futures_prices_daily`

对应 REST 端点：[`GET /api/futures/prices/daily`](/docs/api-reference/futures-prices#prices-daily)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货日K](/docs/api-reference/futures-prices#prices-daily)。

## 期货分时

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_prices_intraday/
...
对应 REST 端点：[`GET /api/futures/prices/intraday`](/docs/api-reference/futures-prices#prices-intraday)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货分时行情](/docs/api-reference/futures-prices#prices-intraday)。

## 期货品种资料

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_varieties_list/

> 查询期货品种、交易所、交易单位与时段等基础资料。

:::info
工具名：`get_futures_varieties_list`

对应 REST 端点：[`GET /api/futures/varieties/list`](/docs/api-reference/futures-varieties)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货品种资料](/docs/api-reference/futures-varieties)。

## 期货品种板块

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_variety_plates_list/

> 查询期货品种板块资料

:::info
工具名：`get_futures_variety_plates_list`

对应 REST 端点：[`GET /api/futures/variety-plates/list`](/docs/api-reference/futures-variety-plates)
:::
...
```text
工具：get_futures_variety_plates_list
参数：
  {}
```
...
返回期货品种板块列表，字段详见 [期货品种板块](/docs/api-reference/futures-variety-plates)。

## 期货历史仓单

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_futures_warehouse_receipts_historical/
...
对应 REST 端点：[`GET /api/futures/warehouse-receipts/historical`](/docs/api-reference/futures-warehouse-receipts)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期货历史仓单](/docs/api-reference/futures-warehouse-receipts)。
...
对应 REST 端点：[`GET /api/meta/tickers/list`](/docs/api-reference/ticker-list)
:::
...
字段含义见 REST 端点 [标的列表获取](/docs/api-reference/ticker-list#响应字段)。
...
对应 REST 端点：[`GET /api/meta/tickers/search`](/docs/api-reference/ticker-search)
:::
...
[标的检索](/docs/api-reference/ticker-search#响应字段)。
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_news_events_search/

> 按标题或正文搜索事件，并按时间、评级、行业和概念代码过滤

:::info
工具名：`get_news_events_search`

对应 REST 端点：[`GET /api/news/events/search`](/docs/api-reference/news-events)

仅同花顺 AI 客户端内可用，不支持 API Key 端外调用。
:::

## 工具描述

> 搜索资讯事件库；仅开放事件标题、正文摘要、时间、评级和关联板块筛选。

## 参数

| 参数                    | 类型    | 必填 | 默认         | 说明                              |
| ----------------------- | ------- | ---- | ------------ | --------------------------------- |
| `query`                 | string  | 否   | —            | 搜索关键词，最多 200 字符。       |
| `search_param`          | enum    | 否   | `event_name` | `event_name` 或 `event_summary`。 |
| `standard_start_time`   | string  | 否   | —            | 起始日期。                        |
| `standard_end_time`     | string  | 否   | —            | 结束日期。                        |
| `event_grading_score`   | string  | 否   | —            | 逗号分隔评级。                    |
| `related_industry_code` | string  | 否   | —            | 逗号分隔行业代码。                |
| `related_concept_code`  | string  | 否   | —            | 逗号分隔概念代码。                |
| `size`                  | integer | 否   | `20`         | 返回条数，范围 1-100。            |

## 调用示例

```text
工具：get_news_events_search
参数：
  - query: "浙江"
...
返回 `{ total, items[] }`；每项含事件标题、摘要、标准时间、评级、关联行业和概念。详见 [资讯事件库](/docs/api-reference/news-events)。

## 期权会话时间轴

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_options_calendar_session_timeline/
...
对应 REST 端点：[`GET /api/options/calendar/session-timeline`](/docs/api-reference/options-session-timeline)
:::
...
来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_options_contracts_detail/
...
对应 REST 端点：[`GET /api/options/contracts/detail`](/docs/api-reference/options-reference#contracts-detail)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期权合约详情](/docs/api-reference/options-reference#contracts-detail)。

## 期权合约基础信息列表

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_options_contracts_list/
...
对应 REST 端点：[`GET /api/options/contracts/list`](/docs/api-reference/options-reference#contracts-list)
:::
...
返回期权合约基础信息分页列表，字段详见 [期权合约基础资料](/docs/api-reference/options-reference#contracts-list)。

## 期权K线

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_options_prices_daily/

> 查询期权 K 线；端外仅支持日 K，AI 客户端内支持完整周期。

:::info
工具名：`get_options_prices_daily`

对应 REST 端点：[`GET /api/options/prices/daily`](/docs/api-reference/options-prices#prices-daily)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期权日K](/docs/api-reference/options-prices#prices-daily)。

## 期权分时

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_options_prices_intraday/
...
对应 REST 端点：[`GET /api/options/prices/intraday`](/docs/api-reference/options-prices#prices-intraday)
:::
...
```text
工具：get_options_prices_intraday
参数：
  - thscode: "IO2601-C-4000.CFE"
  - session: "intraday"
  - trade_date: "0"
...
返回统一数据对象；字段、空值和数组语义见 [期权分时行情](/docs/api-reference/options-prices#prices-intraday)。

## 期权品种资料

来源：https://fuyao.aicubes.cn/docs/mcp/tools/get_options_varieties_list/

> 查询期权品种、交易所、交易单位与交割方式等基础资料。

:::info
工具名：`get_options_varieties_list`

对应 REST 端点：[`GET /api/options/varieties/list`](/docs/api-reference/options-varieties)
:::
...
返回统一数据对象；字段、空值和数组语义见 [期权品种资料](/docs/api-reference/options-varieties)。
