...
安装后执行 `westock --help` 验证；刚安装完命令找不到时，先 `source` 对应 shell profile（如 `~/.zshrc`）使 PATH 生效，再用 `westock` 调用。

> **`westock: command not found` 时**：重跑 `bash scripts/setup.sh`（幂等，会覆盖安装）。不要用 `export PATH`、猜路径或 `find /` 找二进制。重装后仍找不到则如实告知，不要反复重装。
...
- 统一 Go CLI；渠道发布包内二进制通常为 `westock`
- 无脚本入口，只能通过 `westock` 命令调用
- 需网络
...
3. **货币单位随市场**（港元/美元），禁用人民币符号。
4. **K 线有延迟**——`kline` 非实时，展示须标数据日期，勿称「现价」「实时涨跌」。
5. **多股批量**——支持批量的命令一次逗号传多 code，禁止同轮里一部分批量、一部分拆开。单代码例外见下方「批量例外」。
6. **代码必带市场前缀**（`sh/sz/bj`·`hk`·`us`·`fu`·`fx`·`pt`），禁止裸数字 `300685`、Wind 格式 `601988.SH`、港股漏前缀 `00700`。
...
1. **`command not found`**：重跑 `bash scripts/setup.sh` 后重试。
2. **命令失败（其他）**：如实转述，禁止编造数据。
3. **空结果**：说明「暂无数据」；区分代码不支持 vs 时点无披露（必要时先 `westock search`）。
4. **能力不支持**：如实告知（如美股无 `westock fund flow`），见 [routing-guide.md §六](./references/routing-guide.md#六能力差异速查标的--维度)。
5. **禁止**：失败后改用 `web_search` 或凭训练数据补数。
