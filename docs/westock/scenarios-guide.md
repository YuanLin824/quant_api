...
> 命令：`westock lhb` `westock market-overview` `westock connect` `westock ipo`（近几日涨跌可参考 `westock kline --period day --limit 5`；**K 线有延迟，勿当实时价**）
...
→ westock search 腾讯控股 → hk00700
→ westock kline hk00700 --period day --limit 5
...
### 3.3 今日市场点评（market-overview summary）
...
> ⚠️ **职责边界**：`westock connect` = 沪深港通**标的池**（标的列表）；`westock fund flow` = 日度资金流量；`westock fund north-holding` = 北向季度持仓（个股或申万行业）；三者不要混用。

---

## 四、指数（清单/搜索/成份股/行情对比）

> 命令：`westock index list/constituent` + `westock search --type index`；指数走势用 `westock kline`
...
→ westock kline sh000001 --period day --limit 5
→ 解析最近几根日 K 的涨跌幅、成交额 → 标注数据日期
（更广视角：westock market-overview 取 summary / updown 维度）
```
...
→ westock kline sh000001,sz399001 --period day --limit 5
...
⚠️ 错误做法：默认 `westock search 华为` 只搜股票，搜不到板块
...
> **关键区分**：
>
> - `westock search 华为 --type sector` → 拿到板块代码后，用 `westock kline`/ 查板块行情
...
> ⚠️ 易混淆：用户问"板块成份股"用 `westock sector constituent <代码>`；问"板块估值/盈利预测/财务"用 `westock sector valuation/forecast/finance <代码>`；问"板块概况"用 `westock sector info <代码>`。
...
⚠️ 错误做法：
- 用 `westock finance sh600036`（个股三大表）回答行业问题
- 用 `westock consensus`（个股一致预期）代替行业预测
- 用 `westock market-overview --type valuation`（全市场中证全指）代替单行业估值
- 概念/地域板块**不支持** `westock sector forecast` / `westock sector finance`（会明确报错，仅申万 sw1/sw2/sw3 支持）；需改用对应申万行业代码（先 `westock search <关键词> --type sector` 取申万行业 pt 码）
...
4. 可选历史区间：westock sector finance pt01801780 --start 2020-01-01 --end 2026-03-31
...
> 命令：`westock score` `westock rating` `westock consensus` `westock report`
...
> 命令：`westock risk` `westock calendar` `westock suspension`
...
> ⚠️ `westock calendar` 是**个股事件日历**（查某天哪些股票有分红/财报/IPO等事件），不含宏观经济指标。
...
```
用户："分析贵州茅台的盈利能力"
→ westock finance sh600519 → 提取关键指标 → 计算同比/环比 → 输出分析结论
```
...
## 十四、期货（外盘商品/金融期货 + 港股股指期货）

> 命令：`westock futures detail` + `westock search --type futures` + `westock kline`（支持期货代码）

### 14.1 期货行情查询（关键词→代码→行情）
...
## 十五、外汇（离岸人民币/主要货币对/美元指数）

> 命令：`westock forex list` + `westock search --type forex` + `westock kline`（支持外汇代码 `fx*`）

### 15.1 外汇行情查询（关键词→代码→行情）
...
> 命令：`westock kline`（支持可转债代码 沪 `sh11xxxx`/`sh13xxxx`、深 `sz12xxxx`）+ `bond`（发行要素/条款/现金流）

### 16.1 可转债行情查询（价格 + 转债维度）
