...
> ⚠️ **参数不通用，勿凭习惯假设**：`--limit` / `--offset` / `--date` / `--start` / `--end` / `--period`
> 并非所有命令都支持，且 `--limit` 语义随命令而异（`kline`=条数、`finance`=期数）。
> 传错会报 `unknown flag` 并列出该命令真实支持的参数——**按报错修正即可，不要反复试**。
> 不确定某命令支持什么时执行 `westock <命令> --help`。
...
> **统一 `westock search`**：**默认仅搜股票**（A股/港股/美股个股）；用 `--type etf|bond|sector|index|futures|forex` 切换到其它类型，`--type` 支持**逗号分隔多个类型同时搜**（如 `--type etf,index,sector`，按类型分组输出）。`--market` 支持 `hs|bj|hk|us|jp|kr` 六个市场（`jp`/`kr` 走日韩股专用接口）；不传时默认搜索已覆盖 A股/港股/美股。

---

## 二、技术分析

> 衍生指标与筹码分布（`westock technical`/`westock chip`）。

### 技术指标与筹码

```bash
westock technical sh600000                                       # 日K技术指标（默认全量 MA/MACD/KDJ/RSI/BOLL）
westock technical sh600000 --period week                         # 周K技术指标
westock technical sh600000 --period month --limit 12             # 月K近12期
westock technical sh600000 --start 2026-02-01 --end 2026-03-01 --limit 30    # 历史区间
...
```bash
westock lhb                                  # 全市场龙虎榜（默认，tab=all，当日全部上榜股票）
westock lhb --type all --date 2026-03-20     # 显式指定全市场龙虎榜
westock lhb --type institution      # 分类榜（机构榜/活跃席位/高胜率买入/高胜率席位）
...
> ⚠️ **一级 `lhb` vs `fund lhb` 差异**：`westock lhb`（本段）＝ **全市场龙虎榜榜单**，按 `--type`（机构榜/活跃席位等）筛选，**不带个股代码**；`westock fund lhb <代码>`（见 §九 资金）＝ **指定个股**的龙虎榜明细（成交统计 `LhbInfos` + 营业部买卖 `LhbTradingDetails`）。二者不要混用。
...
> 涨跌分布为沪深A股全市场截面（实时）：概览含上涨/下跌/平盘、涨停/跌停、停牌家数与上涨占比情绪文案；明细为 11 个涨跌幅区间（涨停→>7%→…→平→…→跌停）的家数分布，并附两市成交额及其较上日变动。涨跌分布按交易日更新、T+1 生效，回答须标注数据日期。

---

## 四、指数

> 指数清单、搜索与成份股（A 股 + 港股）。K 线复用 `westock kline`。
...
> ⚠️ **板块代码不可臆造**：`pt` 前缀代码必须先用 `westock search <板块名> --type sector` 获取，
> **禁止凭行业名拼接**（臆造代码一律返回「未找到板块」）。所有板块类命令（估值/成份/财务/预期）同理。
...
> - `westock sector info <代码>`：返回板块**信息与交易数据**，**不含成份股**，适合用户问"XX板块怎么样"等总览类问题。**字段随板块类型而异**：概念/产业板块（pt02 等）含名称、成份股数量、区间涨跌幅/成交额、市值与 TTM 财务；申万行业（pt01 开头）上游仅提供名称与成份股数量，涨跌幅/资金流请用 `westock sector ranking --kind industry`。
>
...
```bash
westock sector ranking                     # 行业涨幅榜（默认：industry + changePct + 降序）
westock sector ranking --kind concept      # 概念板块榜
westock sector ranking --type mainNetInflow        # 行业榜按主力净流入排序
westock sector ranking --kind concept --type mainNetInflow --order asc   # 概念榜按主力净流入升序（净流出最多在前）
westock sector ranking --order asc         # 涨幅升序（跌幅榜）

```

**参数**：

- `--kind`：`industry`（行业，默认）/ `concept`（概念）
- `--type` 排序字段：`changePct`（涨跌幅，默认）/ `turnover`（成交额）/ `mainInflow`（主力流入）/ `mainOutflow`（主力流出）/ `mainNetInflow`（主力净流入）/ `mainNetInflow5d`（5日主力净流入）/ `mainNetInflow20d`（20日主力净流入）
- `--order`：`desc`（降序，默认）/ `asc`（升序）

> 金额字段（turnover/mainInflow/mainOutflow/mainNetInflow 等）为**原样数值**（万元），不做单位换算。

> **`westock sector ranking` vs `westock sector constituent/info` 区分**：
>
> - `westock sector ranking`：**全市场板块行情榜**（行业/概念，按涨跌幅/成交额/主力资金排序），用于"今天哪些板块在涨/资金在流向哪"类问题
> - `westock sector constituent/info`：**按代码精查**（成份股 / 画像 / 区间交易），用于"消费板块成份股有哪些 / 半导体板块怎么样"类问题
> - 决策入口：泛问"市场上哪些板块涨得多/资金流向" → `westock sector ranking`；指定"XX 板块的成份股/画像" → `westock sector constituent/info`
...
> 公告详情：`westock notice detail` 返回全文（沪深纯文本；港美 PDF URL）。
...
# 港股：卖空数据（单日 --date / 区间 --start..--end，均支持多代码批量）
westock fund short hk00700
westock fund short hk00700 --date 2026-05-20
westock fund short hk00700 --start 2026-05-01 --end 2026-05-20

# 美股：卖空数据（单日 --date / 区间 --start..--end，均支持多代码批量）
westock fund short usAAPL
westock fund short usAAPL --date 2026-05-20
westock fund short usAAPL --start 2026-05-01 --end 2026-05-20

# 大宗交易（仅沪深）
westock fund block sh600519
westock fund block sh600519,sz000651
westock fund block sh600519 --date 2026-03-20                    # 指定单日
westock fund block sh601318 --start 2026-06-11 --end 2026-08-20   # 时间区间（逐交易日返回，仅含有成交的日期）
...
westock fund margin sh600519 --date 2026-03-20                   # 指定单日（该日未披露会明确报 NOT_DISCLOSED）
westock fund margin sh600519 --start 2026-08-13 --end 2026-08-20  # 时间区间（逐交易日序列，算趋势用这个）
...
> ⚠️ **命令区分**：`flow` = 资金流向（主力/散户）；`short` = 卖空；`block` = 大宗交易；`margin` = 融资融券；`westock fund north-holding` = A股北向**季度持仓**；`south-holding` = 港股南下**持仓快照**
...
> ⚠️ **市场限制**：`block`/`margin` 仅支持沪深（sh/sz）；`short` 支持港股和美股；`flow` 支持沪深和港股（美股请用 `westock fund short`）
> ⚠️ **资金净额与时效**：N 日主力/北向净额直接取接口聚合字段（如 `MainNetFlow5D`），勿对单日数据二次累加；`margin` 按交易日披露、T+1 更新，回答须标注数据日期。
...
> **币种**：财报金额单位随上市地币种（A股人民币/港股港元/美股美元），读 `CurrencyType`/`DisclosureCurrency` 字段，勿默认人民币。
...
> 沪深场内 ETF（仅 `sh`/`sz`）。**子命令必填**；清单 `etf_*` 为主。全市场排行 → `westock etf rank`。

```bash
westock etf profile sh510300
westock etf overview sh510300
westock etf holdings sh510300
westock etf holdings sh510300 --limit 10   # 仅前 10 大持仓（按权重，支持 --offset）
...
> 完整短名表（含中文名称、分组、查询方式）见 [macro-fields.md](./macro-fields.md)。
...
> 专业研究场景（宏观指标组合）见 [scenarios-guide.md](./scenarios-guide.md) 第十四章（14.1~14.23）。
...
> ⚠️ **不支持国内商品期货**：不覆盖上期所/大商所/郑商所/中金所的国内商品期货（沪金、螺纹钢、铁矿石、沪铜等）。用户问国内商品期货须明确告知不支持；`search` 可能模糊命中同名外盘品种（如查"黄金"命中外盘 `fuGC`），回答前须核对交易所与品种，**不得用外盘品种冒充国内品种**。
...
> ⚠️ **期货限制**：外盘期货多为延时行情。`westock kline` 支持 `fu*`、`r_hd*` 合约；`hf_*`（LME）请用 `westock search --type futures` 找代码。期货不支持复权（`--fq` 无效）。
...
> ⚠️ **外汇限制**：外汇不支持复权。品种代码请用 `westock search --type forex` 或 `westock forex list`。

## 十六、债券（可转债）

> 沪深可转债。标准代码：沪市 `sh11xxxx`（110/111/113 可转债、118 科创板可转债）；深市 `sz12xxxx`（123/127/128 可转债）。
...
> ⚠️ **可转债说明**：完整发行要素用 `bond`；K 线用 `westock kline`。
