...
>
> 限流在全局默认（每 60 秒 60 次）之上按接口差异化：注册/改密每小时 5 次、登录每 10 分钟 10 次、
> 刷新每 10 分钟 20 次，两个登出接口豁免。登录另有**账号锁定机制**（见 `login`），与限流是两套独立机制。
...
- `401` - `登录状态已失效, 请重新登录`（旧 token 已轮换重用、已登出、jti 伪造、用户已删除）；
  或 `无效的刷新令牌`（Authorization 头缺失/格式错误、验签失败、载荷校验失败——由守卫抛出）
- `403` - 账号已被禁用
- `429` - 触发接口限流（每 10 分钟 20 次）
...
- `400` - 请求参数校验失败（如缺少 `refreshToken` 字段；幂等仅针对令牌内容，字段本身仍必填）
...
- `401` - `访问令牌无效或已过期`（Authorization 头缺失/格式错误、验签失败、载荷校验失败——由守卫抛出），
  或 `用户不存在或已注销`
- `403` - 账号已被禁用
