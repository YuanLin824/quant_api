...
> 上表中所有接口**均继承全局限流**，没有豁免项。唯一的接口级豁免是 `POST /api/auth/logout`
> 与 `POST /api/auth/logout-all`（登出是幂等的清理动作，豁免以放开登录/登出节奏）。
>
> 行情类接口虽然底层分别是 TCP 长连接（TDX）与子进程（westock CLI），
> 但对外仍是普通重接口，故不开豁免——它们会遍历全市场写库或起子进程。
