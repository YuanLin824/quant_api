...
11. **K 线模块**: `StockKlineService` 提供两条**互不依赖**的取数路径（`src/stock-kline/`），且**用的是两个不同的数据源**——落库（`sync`／`getStats`）走 `TdxService`，实时（`getRealtime`，供 `GET /api/stock-kline`）走 `WestockCliService`。
    ...
    - **聚合日期必须归一化再返回**：`stats` 的 `MIN/MAX(trade_date)` 经 raw query 回来的是 **`Date` 对象**（pg 驱动按本地零点解析，`getRawOne<T>` 的泛型**挡不住**运行时类型），直接交给 JSON 序列化会变成 UTC ISO 串、在 UTC+8 下看起来早一天。故经 `toDayString()` 按本地时区取 `YYYY-MM-DD`——与响应契约（日期一律为 `YYYY-MM-DD` 字符串）对齐
